...
If npm reports a version older than `1.0.0`, the GitHub source is ahead of the published package and `npx @keweixin/code-memory@latest` will not include the v1 global MCP watch routing, multi-repo project selection, structured real-repo benchmark metrics, structured MCP tool results, graph edge evidence contracts, allowed-read constraints, benchmark gate fixes, CLI mirror hardening, lifecycle command coverage, or strict setup auto-agent selection yet.
