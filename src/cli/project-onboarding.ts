...
function buildProjectContextBlock(projectRoot: string): string {
  return [
    ...
    '',
    'Default chain: resolve_project -> bootstrap_project/sync_project/register_project when needed -> resolve_project -> plan_context -> get_context_pack/search_code -> search_symbols -> find_definition/find_references -> impact_analysis -> get_related_tests -> mark_context_used/get_context_delta.',
    '',
    '1. `resolve_project` - verify project identity, db path, readiness, and the next action.',
    '2. `bootstrap_project` / `sync_project` / `register_project` - repair missing, stale, or unregistered project state from MCP before broad reads.',
    '3. `plan_context` - classify the task, check index/vector/ledger readiness, and choose retrieval routes.',
    '4. `get_context_pack` or `search_code` - retrieve bounded evidence for the current task.',
    '5. `search_symbols` then `find_definition` / `find_references` - drill into a specific symbol.',
    '6. `impact_analysis` - run before editing shared symbols, public contracts, or startup/index lifecycle code.',
    '7. `get_related_tests` - identify narrow validation targets.',
    '8. `mark_context_used` / `get_context_delta` - record manually returned context and prefer novel evidence in long sessions.',
    '9. `remember_project_fact` - save durable architecture decisions or bug root causes.',
    '',
    '## Read/Grep Budget',
    '',
    '- Before Read/Grep/Glob, call `resolve_project`, `plan_context`, and `get_context_pack`.',
    '- Only use Read on files returned by Code Memory when extra source detail is needed.',
    '- If Code Memory reports missing/stale/unregistered state, call the MCP management tool first instead of falling back to broad shell search.',
    '',
...
function buildExploringSkill(): string {
  return [
    ...
    '## Tool Order',
    '',
    '1. `resolve_project({ repo?, project? })`',
    '2. If needed: `bootstrap_project`, `sync_project`, or `register_project`, then `resolve_project` again',
    '3. `plan_context({ query })`',
    '4. `get_context_pack({ query, tokenBudget, avoidRepeated: true })`',
    '5. `search_symbols` for named functions/classes/types',
    '6. `find_definition` for exact source locations',
    '7. `get_context_delta` before sending more context for the same session',
    '8. `get_call_graph` or `get_dependency_graph` when relationships matter',
    '',
    '## Done checklist',
    '',
    '- Prefer indexed evidence before broad grep or recursive reads.',
    '- Use Read only on files returned by Code Memory unless the index is missing/stale and cannot be repaired.',
    '- Cite files and symbols from tool output before making claims.',
    '- Save reusable findings with `remember_project_fact`.',
    '- Use `mark_context_used` when context was sent outside `get_context_pack`.',
  ].join('\n') + '\n';
}
...
function buildDebuggingSkill(): string {
  return [
    ...
    '1. `resolve_project({ repo?, project? })`',
    '2. If needed: `bootstrap_project`, `sync_project`, or `register_project`, then `resolve_project` again',
    '3. `plan_context({ query, intent: "debug" })`',
    '4. `get_context_pack` for likely files and prior memories',
    '5. `search_code` for error text, route names, or failing symbols',
    '6. `find_references` / `get_call_graph` to trace callers and callees',
    '7. `get_related_tests` for narrow verification',
    '8. `mark_context_used` for manually returned evidence in long debug sessions',
    '',
    '## Done checklist',
    '',
    '- If stale diagnostics appear, call `sync_project`; if the index is missing, call `bootstrap_project`.',
    ...
    '- Use `get_context_delta` before adding more files to a long session.',
  ].join('\n') + '\n';
}
...
function buildImpactSkill(): string {
  return [
    ...
    '## Tool Order',
    '',
    '1. `resolve_project({ repo?, project? })` and repair state with management tools if needed.',
    '2. `search_symbols` or `find_definition` to identify the exact target.',
    '3. `impact_analysis({ target })` before editing.',
    '4. `get_related_tests` for validation targets.',
    '5. `mark_context_used` if you manually summarize affected context for the agent.',
    '6. After edits, rerun targeted tests and inspect affected files.',
    '',
    '## Done checklist',
    '',
...
function buildRefactoringSkill(): string {
  return [
    ...
    '',
    '1. `resolve_project({ repo?, project? })` and repair state with management tools if needed.',
    '2. `plan_context({ query, intent: "refactor" })`',
    '3. `search_symbols` and `find_references` for all known uses.',
    '4. `impact_analysis` before changing the target.',
    '5. `get_context_delta` to avoid re-sending repeated candidate files.',
    '6. `get_related_tests` and repository-native tests after edits.',
    '',
    '## Done checklist',
    '',
