...
function buildProjectContextBlock(projectRoot: string): string {
  return [
    ...
    '',
    'Default chain: resolve_project -> plan_context -> get_context_pack/search_code -> search_symbols -> find_definition/find_references -> impact_analysis -> get_related_tests -> mark_context_used/get_context_delta.',
    '',
    '1. `resolve_project` - verify project identity, db path, readiness, and bootstrap/index command when missing.',
    '2. `plan_context` - classify the task, check index/vector/ledger readiness, and choose retrieval routes.',
    '3. `get_context_pack` or `search_code` - retrieve bounded evidence for the current task.',
    '4. `search_symbols` then `find_definition` / `find_references` - drill into a specific symbol.',
    '5. `impact_analysis` - run before editing shared symbols, public contracts, or startup/index lifecycle code.',
    '6. `get_related_tests` - identify narrow validation targets.',
    '7. `mark_context_used` / `get_context_delta` - record manually returned context and prefer novel evidence in long sessions.',
    '8. `remember_project_fact` - save durable architecture decisions or bug root causes.',
    '',
...
function buildExploringSkill(): string {
  return [
    ...
    '5. `get_context_delta` before sending more context for the same session',
    '6. `get_call_graph` or `get_dependency_graph` when relationships matter',
    '',
    '## Done checklist',
    '',
...
function buildDebuggingSkill(): string {
  return [
    ...
    '6. `mark_context_used` for manually returned evidence in long debug sessions',
    '',
    '## Done checklist',
    '',
...
function buildImpactSkill(): string {
  return [
    ...
    '4. `mark_context_used` if you manually summarize affected context for the agent.',
    '5. After edits, rerun targeted tests and inspect affected files.',
    '',
    '## Done checklist',
    '',
...
function buildRefactoringSkill(): string {
  return [
    ...
    '4. `get_context_delta` to avoid re-sending repeated candidate files.',
    '5. `get_related_tests` and repository-native tests after edits.',
    '',
    '## Done checklist',
    '',
