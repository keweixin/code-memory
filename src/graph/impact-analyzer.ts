...
export class ImpactAnalyzer {
  ...
  private resolveTargetId(target: string): string | null {
    return this.findFileId(target)
      || this.findSymbolId(target)
      || this.findQualifiedSymbolId(target);
  }

  private findFileId(path: string): string | null {
    try {
      const normalizedPath = path.replace(/\\/g, '/');
      const result = this.db.exec('SELECT id FROM files WHERE path = ?', [normalizedPath]);
  ...
  private findQualifiedSymbolId(name: string): string | null {
