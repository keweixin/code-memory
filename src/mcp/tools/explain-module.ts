    ...
    async ({ filePath, repo }) => {
      try {
        return await withRepoDatabase(repo, db, async (activeDb) => {
          ...
          const symbols = getFileSymbols(activeDb, fileInfo.id);
          const dependencies = getFileDependencies(activeDb, fileInfo.id);
          const dependents = getFileDependents(activeDb, fileInfo.id);
          const chunks = getFileChunks(activeDb, fileInfo.id);
