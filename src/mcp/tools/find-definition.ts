  ...
  server.tool(
    ...
    async ({ symbolName, filePath }) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Error: Find definition failed - ${msg}` }],
          isError: true,
        };
      }
    },
