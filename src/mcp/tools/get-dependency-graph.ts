...
export function registerGetDependencyGraphTool(server: McpServer, db: SqlJsDatabase): void {
  ...
  server.tool(
    ...
    async ({ filePath, depth }) => {
      try {
        const fileId = findFileId(db, filePath);
        if (!fileId) {
          // Try as partial path match
          const matches = findFilesByPartialPath(db, filePath);
          if (matches.length === 0) {
            return {
              content: [{
                type: "text" as const,
          ...
          return analyzeGraph(db, graphEngine, matches[0].id, matches[0].path, Math.min(Math.max(depth, 1), 3));
        }

        return analyzeGraph(db, graphEngine, fileId, filePath, Math.min(Math.max(depth, 1), 3));
      } catch (err) {
