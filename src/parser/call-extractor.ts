...
import type { Node, QueryMatch, Language as TSLang, Query as QueryType } from 'web-tree-sitter';
...
// Lazy-loaded Query class from web-tree-sitter
let _QueryClass: typeof QueryType | null = null;

async function getQueryClass(): Promise<typeof QueryType> {
  if (!_QueryClass) {
    const mod = await import('web-tree-sitter');
    _QueryClass = mod.Query;
  }
  return _QueryClass!;
}
...
function extractCallParts(callNode: Node, lang: ParserLanguage): ExtractedCallParts | null {
  ...
  if (firstChild.type === 'attribute') {
    const objectNode = firstChild.childForFieldName('object') || firstChild.namedChildren[0] || null;
    const attributeNode = firstChild.childForFieldName('attribute') || firstChild.namedChildren[firstChild.namedChildren.length - 1] || null;
    const memberName = attributeNode?.text.trim() || null;
    if (!memberName) return null;
    const receiverName = simplifyReceiverName(objectNode);
    return {
      calleeName: memberName,
      receiverName,
      receiverKind: receiverName && /^[A-Za-z_$][\w$]*$/.test(receiverName) ? 'identifier' : 'unknown',
      memberName,
      isConstructorCall: false,
    };
  }
 ...
 */
export async function extractCallReferences(
  rootNode: Node,
  sourceCode: string,
  lang: ParserLanguage,
  tsLang: TSLang,
  ...
  let q: QueryType;
  try { q = new (await getQueryClass())(tsLang, qs); }
