...
import type { Parser as ParserType, Language as TreeSitterLanguage } from "web-tree-sitter";
import { ParserLanguage, LANGUAGE_CONFIGS, type LanguageConfig } from "./types.js";
import { createLogger } from "../shared/logger.js";

const log = createLogger("parser-registry");

// Lazy-loaded web-tree-sitter module — avoids pulling the WASM bundle
// into the CLI startup path when only lightweight commands (doctor, status)
// are invoked.  The module is loaded on first use by `ensureTreeSitterModule()`.
type WtsModule = typeof import("web-tree-sitter");
let _wtsModule: WtsModule | null = null;

async function ensureTreeSitterModule(): Promise<WtsModule> {
  if (!_wtsModule) {
    _wtsModule = await import("web-tree-sitter");
  }
  return _wtsModule;
}
...
const parserPool = new Map<ParserLanguage, ParserType>();
...
export async function initTreeSitter(): Promise<void> {
  ...
  const { Parser } = await ensureTreeSitterModule();
  await Parser.init();
...
export async function loadLanguage(
  ...
  try {
    const { Language: TreeSitterLanguage } = await ensureTreeSitterModule();
 ...
 */
export function getParser(lang: ParserLanguage): ParserType | null {
  const language = loadedLanguages.get(lang);
  if (!language) return null;

  let parser = parserPool.get(lang);
  if (!parser) {
    // Parser class is guaranteed to be available because loadLanguage()
    // (which calls ensureTreeSitterModule()) must be called first.
    if (!_wtsModule) return null;
    parser = new _wtsModule.Parser();
