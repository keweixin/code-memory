...
import { extractRouteEndpoints, extractRouteReferences } from './route-extractor.js';
...
export async function parseFile(
  ...
  const fileContentHash = contentHash(sourceCode);
  ...
  if (!getParser(parserLang)) {
    try { await loadLanguage(parserLang); }
      ...
      log.error(msg, err);
      return createEmptyResult(fileId, filePath, fileContentHash, msg);
    }
  }
  ...
  if (!parser || !tsLang) {
    return createEmptyResult(fileId, filePath, fileContentHash, 'Parser not available for ' + parserLang);
  }
  ...
  if (!tree) return createEmptyResult(fileId, filePath, fileContentHash, '', true);
  ...
  try { imports = await extractImports(rootNode, sourceCode, parserLang, tsLang); }
  catch (err) { log.error('Import extraction failed', err); }

  let exports: string[] = [];
  try { exports = await extractExports(rootNode, sourceCode, parserLang, tsLang); }
  catch (err) { log.error('Export extraction failed', err); }

  let calls: ParseResult['calls'] = [];
  try { calls = await extractCallReferences(rootNode, sourceCode, parserLang, tsLang); }
  ...
  let routeEndpoints: ParseResult['routeEndpoints'] = [];
  try { routeEndpoints = extractRouteEndpoints(sourceCode, filePath, fileId, symbols, parserLang); }
  catch (err) { log.error('Route endpoint extraction failed', err); }

  let routeReferences: ParseResult['routeReferences'] = [];
  try { routeReferences = extractRouteReferences(sourceCode, fileId, symbols, parserLang); }
  catch (err) { log.error('Route reference extraction failed', err); }
  ...
  return {
    fileId, filePath, language, contentHash: fileContentHash, symbols, imports, exports, edges: [],
    calls, scopeBindings, typeRelations, routeEndpoints, routeReferences, chunks, errors,
  };
}
...
function createEmptyResult(
  fileId: string, filePath: string, fileContentHash: string, errorMsg: string, noError?: boolean,
): ParseResult {
  const errors: ParseError[] = [];
  if (errorMsg && !noError) {
  ...
  return {
    fileId, filePath, language: 'unknown', contentHash: fileContentHash,
    symbols: [], imports: [], exports: [], edges: [], calls: [],
    scopeBindings: [], typeRelations: [], routeEndpoints: [], routeReferences: [], chunks: [], errors,
  };
}
