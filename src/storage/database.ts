...
export function openExistingDatabase(basePath: string): SqlJsDatabase {
  const nextPath = resolveDbPath(basePath);
  if (!existsSync(nextPath)) {
    throw new Error('No code-memory index found at ' + nextPath + '. Run code-memory index --full for that repository first.');
  }

  const native = new DatabaseConstructor(nextPath);
  const adapter = new NativeDatabaseAdapter(native);
  applyPragmas(native);
  return adapter;
}
...
export function needsReindex(): boolean {
  ...
  return !tableExists(db.native, 'file_imports')
    ...
    || !tableExists(db.native, 'route_endpoints')
    || !tableExists(db.native, 'route_references')
    || !tableExists(db.native, 'graph_edge_evidence')
    || !tableExists(db.native, 'processes')
    || !tableExists(db.native, 'process_steps')
    || !tableExists(db.native, 'communities')
    || !tableExists(db.native, 'community_members')
...
function getSchemaMigrations(native: BetterSqliteDatabase): string[] {
  ...
  migrations.push(...missingColumnMigrations(native, 'context_ledger', [
    ['task_id', 'TEXT'],
    ['repo_root', 'TEXT'],
    ['branch', 'TEXT'],
    ['commit_hash', 'TEXT'],
