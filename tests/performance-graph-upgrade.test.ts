...
describe('large-repo storage and graph upgrade contracts', () => {
  ...
  it('opens a native WAL SQLite database with FTS5 bm25/snippet support', async () => {
    ...
    expect(queryRows("SELECT value FROM index_metadata WHERE key = 'schema_version'")[0]?.[0]).toBe('7');
  ...
  it('prints schema and runtime indexing metadata in status json', async () => {
    ...
    expect(status.indexStatus).toBe('completed');
    expect(status.indexRunId).toMatch(/^full-/);
    expect(status.indexRunMode).toBe('full');
    expect(status.indexStartedAt).toBeTruthy();
    expect(status.indexCompletedAt).toBeTruthy();
    expect(status.lastIndexError).toBe(null);
